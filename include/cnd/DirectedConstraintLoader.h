...
public:
  ...
  void LoadAndAddConstraints(
    ...
    // if (arc_link_mapping) {
    //   // Map directed arcs to link IDs
    //   std::size_t unmapped_count = 0;
    //   for (const auto& constraint : constraints) {
    //     auto key = std::make_pair(constraint.init_node, constraint.term_node);
    //     if (arc_link_mapping->find(key) != arc_link_mapping->end()) {
    //       std::size_t link_id = arc_link_mapping->at(key);
    //       solver.AddLinkCapacityConstraint(
    //         link_id,
    //         constraint.lower_bound,
    //         constraint.upper_bound,
    //         constraint.investment_cost_param);
    //     } else {
    //       unmapped_count++;
    //     }
    //   }
    //   if (verbose_ && unmapped_count > 0) {
    //     std::cout << "WARNING: " << unmapped_count << " arcs not found in mapping"
    //               << std::endl;
    //   }
    // } else {
    //   // Direct addition: use arc ID as link ID
    //   for (const auto& constraint : constraints) {
    //     std::size_t link_id = GenerateArcID(constraint.init_node, constraint.term_node);
    //     solver.AddLinkCapacityConstraint(
    //       link_id,
    //       constraint.lower_bound,
    //       constraint.upper_bound,
    ...
    // if (verbose_) {
    //   std::cout << "Added " << constraints.size()
    //             << " constraints to BilevelCND solver" << std::endl;
    // }
  }

  /**
   * @brief Export directed constraints to CSV file
   *
   * @param filepath Output CSV file path
   * @param constraints Vector of constraints to export
   * @param include_header If true, write column headers
  ...
  //   if (include_header) {
  //     file << "init_node,term_node,lower_bound,upper_bound,investment_cost_param\n";
  //   }

  //   for (const auto& constraint : constraints) {
  //     file << constraint.init_node << "," << constraint.term_node << ","
  //          << std::fixed << std::setprecision(12) << constraint.lower_bound
  //          << "," << constraint.upper_bound << ","
  //          << constraint.investment_cost_param << "\n";
  //   }
  ...
  // /**
  //  * @brief Validate directed constraint values for feasibility
  //  *
  //  * @param constraint Constraint to validate
  //  * @param print_errors If true, print error messages
  //  * @return true if constraint is valid, false otherwise
  //  */
  // static bool ValidateConstraint(
  //   const DirectedLinkCapacityConstraint& constraint,
  //   bool print_errors = true) {
    
  //   if (constraint.lower_bound > constraint.upper_bound) {
  //     if (print_errors) {
  //       std::cerr << "ERROR Arc " << constraint.ToString()
  ...
  //   if (constraint.lower_bound < 0.0) {
  //     if (print_errors) {
  //       std::cerr << "ERROR Arc " << constraint.ToString()
  //                 << ": lower_bound cannot be negative (" 
  //                 << constraint.lower_bound << ")" << std::endl;
  //     }
  //     return false;
  //   }

  //   if (constraint.upper_bound <= 0.0) {
  //     if (print_errors) {
  //       std::cerr << "ERROR Arc " << constraint.ToString()
  ...
  //   // Note: investment_cost_param can be 0 (as in your example CSV)
  //   if (constraint.investment_cost_param < 0.0) {
  //     if (print_errors) {
  //       std::cerr << "WARNING Arc " << constraint.ToString()
  //                 << ": investment_cost_param is negative (" 
  //                 << constraint.investment_cost_param << ")" << std::endl;
  //     }
  //   }
  ...
  // /**
  //  * @brief Create constraint template file with examples for directed networks
  //  *
  //  * @param filepath Output template file path
  //  * @param num_examples Number of example rows to include
  //  * @throws std::runtime_error if file cannot be written
  //  */
  // static void CreateTemplate(const std::string& filepath, int num_examples = 5) {
  //   std::ofstream file(filepath);
  //   if (!file.is_open()) {
  //     throw std::runtime_error("Cannot create template file: " + filepath);
  //   }

  //   file << "# Continuous Network Design Problem - Directed Arc Constraints\n";
  //   file << "# init_node: Origin node of the arc\n";
  //   file << "# term_node: Destination node of the arc\n";
  //   file << "# lower_bound: Minimum capacity\n";
  //   file << "# upper_bound: Maximum capacity\n";
  //   file << "# investment_cost_param: Cost per unit capacity expansion\n";
  //   file << "\n";
  //   file << "init_node,term_node,lower_bound,upper_bound,investment_cost_param\n";

  //   // Example entries
  //   for (int i = 0; i < num_examples; ++i) {
  //     std::size_t from_node = (i % 3) + 1;
  //     std::size_t to_node = ((i + 1) % 3) + 1;
      
  //     if (from_node == to_node) to_node = ((i + 2) % 3) + 1;

  //     double lb = 1000.0 + (i * 100.0);
  //     double ub = lb * 1.5;
  //     double cost = (i % 5 == 0) ? 0.0 : (0.01 + i * 0.01);

  //     file << from_node << "," << to_node << "," 
  //          << std::fixed << std::setprecision(6) << lb << "," << ub << ","
  //          << cost << "\n";
  //   }
  ...
  // /**
  //  * @brief Generate a unique arc ID from init and term nodes
  //  *
  //  * Uses Cantor pairing function for unique integer IDs.
  //  * For small networks: ID = init_node * 1000 + term_node
  //  *
  //  * @param init_node Origin node
  //  * @param term_node Destination node
  //  * @return Unique arc identifier
  //  */
...
private:
  ...
  std::optional<DirectedLinkCapacityConstraint> ParseConstraintLine(
    ...
    try {
      ...
      // if (!ValidateConstraint(constraint, verbose_)) {
